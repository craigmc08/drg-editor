...
fn deserialize_bool_value(_: Curs, _: &Tag, _: u64, _: PropertyContext) -> Result<Value> {
  Ok(Value::Bool)
}
fn deserialize_bool_tag(rdr: Curs, _ctx: PropertyContext) -> Result<Tag> {
  let value = rdr.read_u8()? != 0;
  Ok(Tag::Bool(value))
}
/// # Panics
/// If tag is not Bool variant.
fn serialize_bool_tag(tag: &Tag, curs: Curs, _ctx: PropertyContext) -> Result<()> {
...
fn deserialize_enum_value(rdr: Curs, _: &Tag, _: u64, ctx: PropertyContext) -> Result<Value> {
  Ok(Value::Enum(
    NameVariant::read(rdr, ctx.names).with_context(|| "Enum/Byte.value")?,
  ))
}
fn deserialize_enum_tag(rdr: Curs, ctx: PropertyContext) -> Result<Tag> {
  Ok(Tag::Enum(
...
fn serialize_enum_value(val: &Value, _: &Tag, curs: Curs, ctx: PropertyContext) -> Result<()> {
...
fn serialize_enum_tag(tag: &Tag, curs: Curs, ctx: PropertyContext) -> Result<()> {
